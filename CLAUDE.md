...
The orchestrator leverages the **playwright-cli** skill (`.claude/skills/playwright-cli/`) for UI test automation. Key details:
...
- **Automated specs** (auto-generated by the hook-driven pipeline): Follow existing framework patterns; include proper waits, assertions, and error handling.
...
- Test automation is **hook-driven**: a PostToolUse hook detects test case file writes and triggers the orchestrator to generate Playwright specs, run them, and heal failures automatically. No separate automator agents are needed.
- Both UI and API tests use Playwright as the test framework.
